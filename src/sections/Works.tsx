...
export default function Works({ scrollRef: _scrollRef, onSelectProject }: WorksProps) {
  const sectionRef = useRef<HTMLElement>(null)
  const gridRef = useRef<HTMLDivElement>(null)
  const canvasRefs = useRef<(HTMLCanvasElement | null)[]>([])
  const imageLoadedRef = useRef<boolean[]>(new Array(projects.length).fill(false))
  const imagesRef = useRef<(HTMLImageElement | null)[]>(new Array(projects.length).fill(null))
  const strengthRef = useRef(0)
  const prevScrollYRef = useRef(0)
  const randsRef = useRef<number[][]>(
    projects.map(() => [Math.random(), Math.random(), Math.random(), Math.random()])
  )
